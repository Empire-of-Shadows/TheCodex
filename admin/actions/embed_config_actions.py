...
class EmbedConfigActions:
    ...
    @staticmethod
    async def set_feature_roles(guild_id: int, feature: str, role_ids: List[int]) -> bool:
        """Set which roles can access a feature."""
        gcm = await _get_gcm()
        config = await gcm.get_config(guild_id)
        features = dict(config.embed.get("feature_access", {}))
        features[feature] = [str(r) for r in role_ids]
