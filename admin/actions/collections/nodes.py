...
def paginated_list_node(
    ...
    ``query(guild_id) -> dict`` selects the documents (default
    ``{"guild_id": str(guild_id)}`` - IDs are stored as strings, the ecosystem
    standard; a collection that deviates must pass its own ``query``).
