...
SUGGESTIONS = HelpCategory(
    ...
    body=(
        ...
        "between submissions.\n"
        "\n"
        "**`/suggestions [query] [category] [status] [author]`**\n"
        "Browse the server's suggestions, in a message only you can see. Filter by "
        "category, by status (Pending, Under Review, Approved, Implemented, "
        "Rejected, On Hold), or by who posted it, and page through every match "
        "rather than just the first few. You can change the filters from the "
        "message itself, and **Mine only** narrows it to your own. Each entry shows "
        "its status, category, short ID and vote count.\n"
        "Anything you sent anonymously is not tied to your account, so it will not "
        "show up under Mine only.\n"
        "\n"
...
ADMIN = HelpCategory(
    ...
    body=(
        ...
        "from.\n"
        "\n"
        "**Info board** - all in the panel, under **Info Board**\n"
        "**Post / Update Board** puts the board up in a channel you pick, moves it, "
        "or refreshes the copy already posted. **Board Status** shows where it lives "
        "and whether the saved layout is valid.\n"
        "\n"
        "**Member screening** - `/whitelist <user>`\n"
        "New accounts below the age requirement are removed on join unless they are "
        "whitelisted. One command covers it: give it a user and it shows where that "
        "person stands - on the list or not, how old their account is against what "
        "this server asks for - then offers the one action that fits, whether that "
        "is adding them, taking them off, or putting them back. Browsing the whole "
        "list is in the panel under **New Members**. Take a user ID rather than a "
        "username where you can - usernames are case sensitive.\n"
        "\n"
        ...
        "daily schedule, and clear one member's voting record.\n"
        "\n"
